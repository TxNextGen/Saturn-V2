...
async function loadGames() {
    try {
        console.log('[Games] Fetching games.json...');
        ...
        showError('Failed to load games. Check console for details.');
    }
}


function loadFile(path) {
    console.log('[Games] loadFile called with:', path);
    loadGames();
}


function initializePage() {
    console.log('[Games] Initializing page...');
...
function populateCategories() {
    ...
    const categories = getCategories(gamesData);
    console.log('[Games] Categories:', categories);
...
function filterGames() {
    ...
    if (searchCache.has(cacheKey)) {
        renderFilteredGames(searchCache.get(cacheKey));
        return;
    }
    ...
    console.log('[Games] Filtered to', filtered.length, 'games');
...
function renderFilteredGames(games) {
    ...
    console.log('[Games] Rendering', games.length, 'games');
...
function createGameCard(game, index) {
    ...
    img.src = game.img || 'https://placehold.co/200x200/9333ea/ffffff?text=' + encodeURIComponent(game.title.substring(0, 2));
    img.alt = game.title;
    img.loading = 'lazy';
...
function openGame(game) {
    ...
    console.log(`[Games] Opening: ${game.title}`);
    console.log(`[Games] Game path: ${path}`);
    console.log(`[Games] Full URL: ${window.location.origin}/${path}`);
...
function showNoResults(container) {
    ...
    msg.innerHTML = `
        ...
        <p>Can't find the game you're looking for?<br>Join our Discord to request it!</p>
...
function showError(message) {
    const preload = document.getElementById('preload');
    if (preload) {
        preload.innerHTML = `<p style="color: #ff6b6b;">${message}</p>`;
    }
}
...
window.addEventListener('storage', (e) => {
    if (e.key === 'theme-change-event') {
        try {
            ...
            console.error('[Games] Error parsing theme change:', err);
        }
    }
});
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        console.log('[Games] DOM Content Loaded');
        loadGames();
    });
} else {
    console.log('[Games] Document already loaded');
    loadGames();
}